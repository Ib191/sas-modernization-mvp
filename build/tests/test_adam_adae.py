"""Row-for-row equality test for `adam.adae`.

Auto-generated by Phase 4 from the spec acceptance criteria. Compares
build/target/output/adae.csv against ground_truth/adae.csv.
"""
import pandas as pd
import pytest
...
def test_schema_match(target_output_dir, ground_truth_dir):
    gen = _read(target_output_dir / "adae.csv")
    truth = _read(ground_truth_dir / "adae.csv")
...
def test_row_for_row_equality(target_output_dir, ground_truth_dir):
    gen = _normalize(_read(target_output_dir / "adae.csv"))
    truth = _normalize(_read(ground_truth_dir / "adae.csv"))
