"""Row-for-row equality test for `adam.ae_clean`.

Auto-generated by Phase 4 from the spec acceptance criteria. Compares
build/target/output/ae_clean.csv against ground_truth/ae_clean.csv.
"""
import pandas as pd
import pytest


SORT_KEY = ['USUBJID', 'AESEQ']
...
def test_schema_match(target_output_dir, ground_truth_dir):
    gen = _read(target_output_dir / "ae_clean.csv")
    truth = _read(ground_truth_dir / "ae_clean.csv")
...
def test_row_for_row_equality(target_output_dir, ground_truth_dir):
    gen = _normalize(_read(target_output_dir / "ae_clean.csv"))
    truth = _normalize(_read(ground_truth_dir / "ae_clean.csv"))
