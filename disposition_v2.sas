...
%macro params;
	...
	%if %upcase(&run_location.) = LOCAL %then %do; 
		...
		data _null_;
		    set macrovar;
		    call execute('%nrstr(%symdel) ' || trim(left(name)) || ' / nowarn;');
		run;
		...
		%let studypath =/home/u64525761/phuse/sdtm/;

		/* Assign XPORT libraries pointing directly to each .xpt file */
		libname dmlib xport "&studypath.dm.xpt";
		libname dslib xport "&studypath.ds.xpt";
		libname exlib xport "&studypath.ex.xpt";

		/* Copy the XPT datasets into WORK */
		data dm; set dmlib.dm; run; 
		data ds; set dslib.ds; run; 
		data ex; set exlib.ex; run; 

		/* Release the XPORT librefs */
		libname dmlib clear;
		libname dslib clear;
		libname exlib clear;
		...
		%let saspath = /home/u64525761/phuse/utils/ZZ_Utilities;
		%let utilpath = /home/u64525761/phuse/utils/ZZ_Utilities;

		/* location and filename of the disposition panel template */
		%let templatepath = /home/u64525761/phuse/utils/Templates;
		%let template = Disposition_Template.xls;

		/* location and filename of the output */
		%let outpath = /home/u64525761/phuse/output;
		%let outfile = Disposition.xlsx;

		%global dispout errout;
		%let dispout = &outpath./&outfile.;	
		%let errout = &outpath./Disposition Error Summary.xls;

		/* options noxwait xsync;  -- Windows-only, not valid on SAS OnDemand (Linux) */

		/* copy the template to the output file -- disabled; xlsx engine creates the file directly */
...
%macro ds_out;
	...
	** ADD IN THE DATA INFORMATION ***;
	...
	/* native XLSX engine on SAS OnDemand (Linux) -- replaces EXCEL/PCFILES engines */
	libname xls xlsx "&dispout.";

	/* DROP TABLE block disabled -- XLSX engine does not support DROP TABLE
	proc sql noprint;
